...
Four native inquiry forms have optional Phone fields staged; [task 15 evidence](../task15/README.md) records form IDs, schema/HTTP comparison, responsive/category checks, serialization limits and screenshots. Production fields and original inline scripts are unchanged. The 768px document-width 790 navbar overflow is inherited; new fields fit. Task 37 must still verify actual blank/international submissions through a controlled destination, CAPTCHA and success/error states, downstream webhook/notification mapping after safe backend deployment, and the isolated ActiveCampaign form. No delivery is claimed from local renderer or standard FormData tests. Viewport was reset after task 15.

## Task 28 deferred account acceptance — September 24, 2026

Owner: IPMI account/domain administrator. [Task 28](../../28-activecampaign-domain.md#september-24-2026-investigation-and-pending-replacement) completed read-only investigation, but the known ActiveCampaign admin presents Login. Exact intended/configured domain, domain function, administrator and dated authenticated provider status remain pending. The DORN staging page uses account `ipmionline81168.activehosted.com`, embed form 412 and its `proc.php` submission endpoint; account-side destinations and actual delivery are unverified. Preserve the current integration. The public contact email and native Webflow/Resend notification path do not verify this provider's domain. Required follow-up: administrator evidence and any necessary remediation/retest, then safely isolated affected-form verification coordinated with task 15. No website edit or publish was required by this investigation, and no content placeholder can satisfy this acceptance.
...
[Task 20 evidence](../task20/README.md) records the reproduced mobile link defect and native destination correction. Repeat Home mobile Learn More pointer/keyboard activation after opening and closing the menu in integrated acceptance. 375/390/767px checks passed; desktop/tablet variant preserved. The inherited 768px navbar document width of 790 remains separate from this corrected link. Production mobile href remains '#'.

## Task 24 provisional verification record — September 24, 2026

[Calendar evidence](../task24/README.md) records all26 native data bindings, immutable JS/CSS release, root-independent reconciliation/preservation, released/Horizon/cross-month examples, keyboard/focus/scroll/filter checks and320/390/768/desktop screenshots. Calendar dialog has no horizontal overflow; the inherited768px shared-nav page width790 remains for integrated correction. Five synthetic tests cover date validity, DST/midnight, ongoing/year-crossing, expiry/reopen and state isolation; targeted lint/build/styles passed. Full verify retains the known document-formatting baseline. Final IPMI schedule approval/completeness and label replacement remain task35/PLACEHOLDERS; no production publication or actual form delivery is implied.

## Task 26 agenda handoff — September 24, 2026

[Task26 evidence](../task26/README.md) verifies all54 compact agendas, 14 native event-specific links / 40 pending hidden CTAs, exact six-field CMS delta and all43 retained blocks. Repeat direct hash, desktop pointer/keyboard and mobile select after later template edits, including HCHR390→768 reload: initial hash now realigns after browser restoration without overriding visitor intent. Runtime is immutable `1afeaaadea5b1f0a1e5d87819d2443b224e37a06`; frozen base CSS unchanged, scoped tablet Agenda supplement `b469ddcbe2f81466fa450a1d187a68ef0739b10c` removes obsolete grid height. Operations link/copy approval remains provisional. Task27 owns inherited Hub/support destinations; task37 retains date UTC shift, sparse Experience spacing/empty-state contrast/shared768px navbar, accumulated formatting, and task24 toolchain entry inventory integration. Full tests at26:249/250, with only that existing calendar inventory failure. No production publication.

## Task 27 destination handoff — September 24, 2026

[Task 27 evidence](../task27/README.md) verifies all 54 page/audience rows and 81 pending destination purposes. Four Sponsor example URLs are cleared independently; two useful Delegate mailtos have `EMAIL OPERATIONS` labels and explicit MeetMax-pending/email-purpose copy. No template/runtime/CSS pin changed. Exactly 12 native field deltas, all 43 blocks and all other fields preserved; all 54 HTTP 200/noindex/agenda/images/other links pass. Six representative pages pass 1912/390px layouts, visible keyboard email focus and Hub/Support navigation. Native condition-hidden empty anchors remain `href="#"` in HTML but are not visible or keyboard targets. Actual MeetMax URL/landing approval and Sponsor Support purpose remain external inputs. When replacing Delegate fallback URLs, update their labels and remove the email explanation together; keep staff contacts separate. Staging 13:02:25.687Z; custom domains unchanged 03:17:26.557Z. Existing task 37 date/layout/formatting/toolchain findings remain separate.

## Task 19 accessibility handoff — September 24, 2026

[Task 19 evidence](../task19/README.md) records 27 approved photo placements and 67 active Gallery Photos records / 97 native description bindings. Four pages use immutable photo accessibility release `a3103da593efd7afeed2ba7f6b92a0ec0bb46e9b`; native viewer groups/destinations, keyboard/focus return and descriptions pass at desktop/tablet/mobile. Staging publication 14:23:37.647Z; all custom domains remain 03:17:26.557Z. Toolchain Calendar ownership/shell and CSS dependency walking are now fixed; do not redo that historical integration item.

Preserve the concurrently authorized KBYG edits: the user requires day cards **and** full-agenda PDF button. Rebaseline external EHS copy/references/venue assets and changed TAEEI Canada slugs rather than restoring older task 26/27 records. The three externally modified local KBYG files are deliberately excluded from task 19 commits. The task 19 evidence gives exact IDs and descriptions for one over-specific EHS smartphone hero alt and two new venue images currently falling back to `Event venue`; finish those through appropriate maintained/native inputs and verify linked/full-size names. Also hide inherited Swiper status visually on pages outside the four photo-supplement pages while retaining announcements: homepage keyboard Next visibly exposes `This is the last slide` at 768px. Other shared layout/date/content-approval findings remain unchanged.
