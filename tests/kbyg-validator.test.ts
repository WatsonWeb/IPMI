import fixture from "./fixtures/kbyg-cms-model.json" with { type: "json" };

import { assert, describe, expect, it } from "vite-plus/test";

import { validateKbygModel } from "../scripts/kbyg-validator.ts";

const validModel = () => {
  const model = structuredClone(fixture);
  return {
    ...model,
    pages: model.pages.map((page) => ({ ...page, faqs: Array.from<string>(page.faqs) })),
  };
};
const errorsFor = (model: unknown): string => validateKbygModel(model).join("\n");

describe("validateKbygModel", () => {
  it("rejects malformed external inputs without throwing or trusting JSON record shapes", () => {
    for (const input of [
      null,
      false,
      42,
      ...
      [],
      { pages: 1 },
      { pages: [null], blocks: [false], institutes: [7], staff: ["unknown"] },
    ]) {
      expect(() => validateKbygModel(input)).not.toThrow();
      expect(validateKbygModel(input).length).toBeGreaterThan(0);
    }
    const model = validModel();
    Reflect.set(model.pages[0], "audience", { value: "delegate" });
    Reflect.set(model.blocks[0], "blockType", { value: "preparation" });
    Reflect.set(model.institutes[0], "id", 123);
    expect(errorsFor(model)).toContain("invalid Audience");
    expect(errorsFor(model)).toContain("invalid Block Type");
    expect(errorsFor(model)).toContain("missing id");
  });
  ...
  it("rejects duplicate references, incorrect block types, audience leakage, and cross-Institute blocks", () => {
    ...
    const otherInstituteBlock = model.blocks.find(({ id }) => id === model.pages[0].agendaDays[1]);
    assert.isDefined(otherInstituteBlock);
    otherInstituteBlock.institute = "another-event";
  ...
  it("validates all-day calendar starts and ends as strict, real ISO dates on the same day", () => {
    ...
    Reflect.deleteProperty(dates[2], "allDay");
  ...
  it("allows at most one expanded delegate FAQ and forbids sponsor FAQ references", () => {
    const model = validModel();
    const secondFaq = model.blocks.find(({ id }) => id === model.pages[0].faqs[1]);
    assert.isDefined(secondFaq);
    secondFaq.initiallyExpanded = true;
  ...
  it("requires references, accessible images, alt text, and audience-specific core copy", () => {
    const model = validModel();
    Reflect.deleteProperty(model.pages[0], "operationsLead");
    model.pages[0].heroImageAlt = "";
    Reflect.set(model.pages[0], "hubImage", { fileId: "asset-without-url" });
  ...
  it("validates referenced Institute and Staff records and their shared KBYG fields", () => {
    ...
    Reflect.deleteProperty(institute, "venue");
    ...
    Reflect.set(operationsLead, "profilePhoto", { fileId: "asset-without-url" });
    ...
    Reflect.deleteProperty(missingSharedRecords, "institutes");
    Reflect.deleteProperty(missingSharedRecords, "staff");
