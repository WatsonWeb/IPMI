...
function setOutlineColors(cardWrappers) {
  cardWrappers.forEach((wrapper) => {
    ...
    if (selectorButton) {
      ...
      const accentColor = window.getComputedStyle(selectorButton).backgroundColor;
      // Ensure the fetched color is not transparent or empty before setting
      if (accentColor && accentColor !== "rgba(0, 0, 0, 0)" && accentColor !== "transparent") {
      ...
      if (selectedOption) {
        ...
        if (slugToSelect && cardWrappers.length > 0) {
          cardWrappers.forEach((card) => {
            ...
            if (eventInfo && eventInfo.getAttribute("data-slug") === slugToSelect) {
